...

{
...

    {
...

    	{
            ...
            $inserted_data['shortcode'] = $shortcode;
...

    {
        ...
        $shortcode = isset($get_info[0]['shortcode']) ? $get_info[0]['shortcode']: "";
...

    {
...

        {
            ...
            $shortcode = $post['updated_shortcode'];
            ...
            $updated_data['shortcode'] = $shortcode;
    ...
    public function google_contacts_total_numbers()
    {
        if(!$_POST) exit;
        $this->ajax_check();
        $user_id = $this->user_id;
        $google_contacts_ids = $this->input->post('google_contacts_id', true);
        if (isset($google_contacts_ids) && !empty($google_contacts_ids)) {
            foreach ($google_contacts_ids as  $key => $value) {
                
                $where_simple = array('google_contacts.user_id'=>$this->user_id,"google_contacts.google_contacts_account_id "=>$value);
                $where = array('where'=>$where_simple);    
                $contact_details = $this->basic->get_data('google_contacts', $where,array("phone"));
                foreach ($contact_details as $contact) {
                    $contacts_id[] = isset($contact['id']) ? $contact['id'] : "";
                }
            }
        }
        // Get the total contact count
        $total_contact = isset($contacts_id) ? count($contacts_id) : 0;
        // // Output the total contact count
        echo $total_contact;
...

    {
...

        {
         ...
         if ( $this->basic->is_exist("add_ons",array("project_id"=>72)) && ($this->session->userdata('user_type') == 'Admin' || in_array(353, $this->module_access))){
            ...
            if(isset($google_contacts_sms_group) && !empty($google_contacts_sms_group))
             $google_contact_groupid    = implode(",",$google_contacts_sms_group);
             $google_contacts_id = array();
            if(isset($google_contacts_sms_group) && !empty($google_contacts_sms_group)){
 
                foreach ($google_contacts_sms_group as $key => $value) 
    
                {
                    ...
                    $google_contact_details = $this->basic->get_data('google_contacts', $where);
                    foreach ($google_contact_details as $key3 => $value3) 
                    {   
                        if($value3['phone'] == "") continue;
                        $report[$value3['phone']] = array(
    
                            'api_id'              => $sms_api,
                            'contact_id'          => '0',
                            'subscriber_id'       => "0",
                            'contact_first_name'  => isset($value3['name']) ? $value3['name']:"",
                            'contact_last_name'   => "",
                            'contact_email'       => isset($value3['email']) ? $value3['email']:"",
                            'contact_phone_number'=> isset($value3['phone']) ? $value3['phone']:"",
                            'sent_time'           =>'pending',
                            'delivery_id'         =>'pending',
    
                        );
    
                        $google_contacts_id[] = isset($value3["id"]) ? $value3["id"]: "";
                    }
                   
                }
            }
            $google_contacts_id = array_filter($google_contacts_id);
            $google_contacts_id = array_unique($google_contacts_id);
            $google_contacts_id = implode(',', $google_contacts_id);
        ...
        $inserted_data = array(
            ...
            "google_contacts_id"=> isset($google_contacts_id) ? $google_contacts_id:"",
            'contact_type_id'   => isset($contact_groupid) ? $contact_groupid:"",
            'google_contact_type_id'=>isset($google_contact_groupid) ? $google_contact_groupid:"",
