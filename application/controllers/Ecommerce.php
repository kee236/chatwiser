...
public function change_payment_status()
{
    ...
    $update_data = array(
        ...
        "status_changed_note" => $status_changed_note
    );

    $join = array('ecommerce_store' => "ecommerce_store.id=ecommerce_cart.store_id,left");
    $cart_data = $this->basic->get_data(
        'ecommerce_cart',
        array("where" => array("ecommerce_cart.id" => $id)),
        "buyer_email,buyer_mobile,buyer_first_name,buyer_last_name,bill_email,bill_mobile,ecommerce_cart.id,subscriber_id,page_id,store_name,store_unique_id,ecommerce_store.id as store_id,notification_message,notification_sms_api_id,notification_email_api_id,notification_email_subject,notification_configure_email_table",
        $join
    );
    ...
    if ($notification_email_subject == "") {
        $notification_email_subject = "{{store_name}} | Order Update";
    }

    if (empty($email)) {
        $email = isset($cart_data[0]['bill_email']) ? $cart_data[0]['bill_email'] : '';
    }
    
    if (empty($mobile)) {
        $mobile = isset($cart_data[0]['bill_mobile']) ? $cart_data[0]['bill_mobile'] : '';
    }
    
    $api_response = array();  // Ensure api_response is always an array
    ...
    if ($this->basic->update_data("ecommerce_cart", array("id" => $id, "user_id" => $user_id), $update_data)) {
        ...
        $replace_variables = array(
            ...
            "invoice_url" => $order_url,
            "update_note" => $status_changed_note,
        ...
        if ($messenger_text != "") {
            ...
            if ($social_media_type == 'fb') {
                ...
                $messenger_confirmation_template = array(
                    "recipient" => array("id" => $subscriber_id),
                    "message" => array("text" => $messenger_text . ' ' . $this->lang->line('Order URL') . ': ' . $my_orders_url)
                );
            }
        ...
        if ($email != "" && $notification_email_api_id != '0') {
            ...
            if (trim($email_text) != '') {
                try {
                    ...
                    if (isset($response) && !empty($response) && $response == "Submited") {
                        $message_sent_id = $response;
                        if ($message_sent_id == "Submited") {
                            $message_sent_id = "Submitted";
                        }
                        $email_response = array("response" => $message_sent_id, "status" => '1');
                    } else {
                        $message_sent_id = $response;
        ...
        foreach ($api_response as $key => $value) {
            if ($value['status'] == '1') {
                $api_status = "<span class='badge badge-success'><i class='fas fa-check-circle'></i> " . $this->lang->line("Success") . "</span>";
            } else {
                $api_status = "<span class='badge badge-danger'><i class='fas fa-times-circle'></i> " . $this->lang->line("Error") . "</span>";
            }
        ...
        echo json_encode(array('status' => '1', 'message' => $this->lang->line("Payment status has been updated successfully.") . "<br><br><br>" . $api_response_formatted), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    } else {
