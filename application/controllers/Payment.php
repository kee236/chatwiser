...
class Payment extends Home
{
    ...
    public function accounts_action()
    {
        ...
        if ($_POST) {
            ...
            $this->form_validation->set_rules('meps_public_key', '<b>' . $this->lang->line("Meps Public Key") . '</b>', 'trim');
            $this->form_validation->set_rules('meps_profile_id', '<b>' . $this->lang->line("Meps Profile ID") . '</b>', 'trim');
            ...
            if ($this->form_validation->run() == false) {
                ...
                $meps_public_key = strip_tags($this->input->post('meps_public_key', true));
                $meps_profile_id = strip_tags($this->input->post('meps_profile_id', true));
                ...
                if ($paypal_payment_type == "")
                    $paypal_payment_type = "manual";
                if ($paypal_mode == "")
                    $paypal_mode = "live";
                ...
                $update_data =
                    ...
                    (
                        ...
                        'meps_public_key' => $meps_public_key,
                        'meps_profile_id' => $meps_profile_id,
                ...
                if (isset($get_data[0]))
                    ...
                    $this->basic->insert_data("payment_config", $update_data);
    ...
    public function earning_summary()
    {
        ...
        foreach ($payment_data as $key => $value) {
            ...
            if (strtotime($value['payment_date']) == strtotime($date))
                $payment_today += $value["paid_amount"];
            ...
            if (date("m", strtotime($value['payment_date'])) == $month && date("Y", strtotime($value['payment_date'])) == $year) {
                ...
                if (!isset($array_month[$payment_date]))
                    $array_month[$payment_date] = 0;
            ...
            if (date("Y", strtotime($value['payment_date'])) == $year) {
                ...
                if (!isset($array_year[$mon]))
                    $array_year[$mon] = 0;
                $array_year[$mon] += $value["paid_amount"];

                if (isset($this_year_earning[$mon2 - 1]))
                    $this_year_earning[$mon2 - 1] += $value["paid_amount"];

                if (!isset($this_year_top[$value['country']]))
                    $this_year_top[$value['country']] = 0;
                $this_year_top[$value['country']] += $value["paid_amount"];
            }

            if (date("Y", strtotime($value['payment_date'])) == $lastyear) {
                if (isset($last_year_earning[$mon2 - 1]))
                    $last_year_earning[$mon2 - 1] += $value["paid_amount"];

                if (!isset($last_year_top[$value['country']]))
                    $last_year_top[$value['country']] = 0;
    ...
    public function transaction_log() // works for both admin and member
    {
        ...
        if ($action != "") {
            if ($action == "cancel")
                $this->session->set_userdata('payment_cancel', 1);
    ...
    public function buy_package()
    {
        ...
        if ($this->session->userdata('license_type') == 'double' && $this->session->userdata('user_type') == 'Member') {
            ...
            if (!isset($user_info[0]))
                exit();
            if ($user_info[0]['paypal_subscription_enabled'] == '1')
                $data['has_reccuring'] = 'true';
            else
                $data['has_reccuring'] = 'false';
    ...
    public function manual_payment()
    {
        ...
        if (!empty($mp_resubmitted_id)) {
            ...
            if (
                $this->basic->update_data('transaction_history_manual', $update_where, $update_data)
            ) {
    ...
    public function manual_payment_approve($transaction_id)
    {
        ...
        $payment_confirmation_email_template = $this->basic->get_data(
        ...
        if (
            isset($payment_confirmation_email_template[0])
        ...
        if (
            isset($payment_confirmation_email_template[1])
    ...
    public function payment_button($package_id = 0)
    {
        ...
        if ($this->session->userdata('license_type') == 'double' && $this->session->userdata('user_type') == 'Member') {
            ...
            $pp_button = $meps_button = $st_button = $razorpay_button = $paystack_button = $mollie_button = $mercadopago = $sslcommerz = $senangpay = $instamojo = $xendit = $toyyibpay = $myfatoorah = $paymaya_button = "";
            ...
            if ($meps_profile_id != '' && $meps_public_key != '') {

                $redirect_url_paymaya = base_url('stripe_action/meps_action/') . $package_id . '/' . $this->user_id;
                $this->load->library('meps');
                $this->meps->redirect_url = $redirect_url_paymaya;
                $this->meps->button_lang = $this->lang->line('Pay with Meps');
                $meps_button = $this->meps->set_button();
            }
            ...
            if ($meps_button != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $meps_button . '</div>';
            if ($pp_button != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $pp_button . '</div>';
            if ($st_button != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $st_button . '</div>';
            if ($razorpay_button != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $razorpay_button . '</div>';
            if ($paystack_button != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $paystack_button . '</div>';
            if ($mollie_button != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $mollie_button . '</div>';
            if ($mercadopago != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $mercadopago . '</div>';
            if ($sslcommerz != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $sslcommerz . '</div>';
            if ($senangpay != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $senangpay . '</div>';
            if ($instamojo != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $instamojo . '</div>';
            if ($toyyibpay != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $toyyibpay . '</div>';
            if ($myfatoorah != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $myfatoorah . '</div>';
            if ($paymaya_button != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $paymaya_button . '</div>';
            if ($xendit != '')
                $buttons_html .= '<div class="col-12 col-md-4 pt-3">' . $xendit . '</div>';
            $buttons_html .= '</div>';
    ...
    public function usage_history()
    {
        ...
        if (isset($package_info["monthly_limit"]))
            $monthly_limit = $package_info["monthly_limit"];
        $bulk_limit = array();
        if (isset($package_info["bulk_limit"]))
            $bulk_limit = $package_info["bulk_limit"];
        $package_name = "No Package";
        if (isset($package_info["package_name"]))
            $package_name = $package_info["package_name"];
        $validity = "0";
        if (isset($package_info["validity"]))
