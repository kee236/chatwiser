...
class Fb_autopost{
		...
		if($this->user_id!="")
		{
			...
			if($this->app_id!="" && $this->app_secret!="")
			{
				$this->fb = new Facebook\Facebook([
					...
					'default_graph_version' => 'v20.0', // Updated from v2.2
				]);
			}
	...
	protected function subscriber_info($access_token = '', $sender_id = '', $social_media_type = "fb")
	{
		...
		if ($social_media_type == 'fb')
			$url = "https://graph.facebook.com/v20.0/$sender_id?access_token=$access_token&fields=id,first_name,last_name,name,profile_pic,locale,timezone,gender";
		else
			$url = "https://graph.facebook.com/v20.0/$sender_id?access_token=$access_token&fields=id,name,profile_pic";
		...
		if (isset($result['error']) && $result['error']['code'] == 10 && $result['error']['error_subcode'] == 2018336) {
			$result = array();
			// Updated API version to v20.0
			$url = "https://graph.facebook.com/v20.0/$sender_id?access_token=$access_token&fields=id,first_name,last_name,name,locale,timezone,gender";
			$result = $this->subscriber_info_curl_call($url);
			$result['profile_pic'] = "";
		}
