<script>
  ...
  $(document).on('click','.enable_random_delay',function(e){
    e.preventDefault();
    var page_id = $(this).attr('page_id');
    var enable_random_delay = 0;
    if (this.checked) {
      enable_random_delay = 1;
    } else {
      enable_random_delay = 0;
    }
    ...
    var base_url = '<?php echo site_url();?>';
        $.ajax({
          context: this,
          type:'POST' ,
          url:"<?php echo site_url();?>comment_automation/enable_random_delay",
          dataType: 'json',
          data:{enable_random_delay:enable_random_delay,page_id:page_id},
          success:function(response){ 
            $(this).removeClass('btn-progress');
            if(response.status == '1')
            {
