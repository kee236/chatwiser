<?php
/*
Addon Name: Auto Feed Post
Unique Name: auto_feed_post
Modules:
{
   "269":{
      ...
      "limit_enabled":"1",
      "extra_text":"month",
      "module_name":"Auto Feed - WordPress Feed Post"
   },
   "276":{
      "bulk_limit_enabled":"1",
      "limit_enabled":"1",
      "extra_text":"month",
      "module_name":"Auto Feed - YouTube Video Post"
   }
}
Project ID: 39
...
class Auto_feed_post extends Home
{
    ...
    {   
        ...
        $settings_data=$this->basic->get_data("autoposting",array("where"=>array("user_id"=>$this->user_id,"feed_type"=>"youtube")),'','','','','feed_name asc');
        
        $data['body'] = 'youtube_settings';
        $data['page_title'] = $this->lang->line('Youtube video auto-posting');  
    ...
    public function wordpress_settings()
    {
        ...
        $settings_data=$this->basic->get_data("autoposting",array("where"=>array("user_id"=>$this->user_id,"feed_type"=>"wordpress")),'','','','','feed_name asc');
        $blog_urls = $this->basic->get_data('wordpress_config_self_hosted', ['where' => ['user_id' => $this->user_id, 'status' => '1']], ['domain_name']);

        $wordpress_blogs = [];
        
        if (count($blog_urls) > 0) {
            foreach ($blog_urls as $blog_url) {
                $wordpress_blogs[$blog_url['domain_name']] = $blog_url['domain_name'];
            }            
        }

        $data['body'] = 'wordpress_settings';
        $data['page_title'] = $this->lang->line('Wordpress post auto-posting');  
        $data['page_info'] = isset($page_info[0]) ? $page_info[0] : array();  
        $data['settings_data'] = $settings_data;
    ...
    public function add_feed_action()
    {
        ...
        if ('youtube' == $feed_type) {
            ...
            $youtube_channel_id = trim($this->input->post('youtube_channel_id', true));
            ...
            if (! $youtube_api_key) {
                $error_message = isset($feed['error_message'])
                    ? $feed['error_message']
                    : $this->lang->line("YouTube API key is not set. Please contact the admin");

                echo json_encode(array('status' => '0', 'message' => $error_message));
                exit();
            }

            $insert_data['youtube_channel_id'] = $youtube_channel_id;
            ...
            $wordpress_blog_url = trim($this->input->post('wordpress_blog_url', true));

            if (! filter_var($wordpress_blog_url, FILTER_SANITIZE_URL, FILTER_VALIDATE_URL)) {
                $error_message = $this->lang->line("Please provide a valid url for your blog");
                echo json_encode(array('status'=>'0','message'=>$error_message));
                exit();
            }

            $insert_data['wordpress_blog_url'] = $wordpress_blog_url;
        ...
        $this->basic->insert_data("autoposting", $insert_data);

        if ($this->db->affected_rows() > 0) {

            if ($module_id) {
                $this->_insert_usage_log($module_id, $request=1);
            }
    ...
    public function enable_settings()
    {
        ...
        if ('rss' == $media_type) {
            ...
            $youtube_channel_id = isset($get_data[0]['youtube_channel_id']) 
                ? $get_data[0]['youtube_channel_id'] 
                : '';
            $google_settings = $this->basic->get_data('login_config', '', ['api_key']);
            $youtube_api_key = isset($google_settings[0]['api_key']) 
                ? $google_settings[0]['api_key'] 
            ...
            $this->load->library('youtube_channel');
            $feed = $this->youtube_channel->get_videos($youtube_channel_id, $youtube_api_key);
        } elseif ('wordpress' == $media_type) {
            $wordpress_blog_url = isset($get_data[0]['wordpress_blog_url'])
                ? $get_data[0]['wordpress_blog_url']
                : '';
