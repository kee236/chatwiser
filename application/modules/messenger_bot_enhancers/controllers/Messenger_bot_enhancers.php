...
Modules:
{
   "211":{
      ...
      "limit_enabled":"1",
      "extra_text":"month",
      "module_name":"Bot - Enhancers : Broadcast : Subscriber Bulk Message Send"
   },
   "213":{
      "bulk_limit_enabled":"0",
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"Bot - Enhancers : Engagement : Checkbox Plugin"
   },
   "214":{
      "bulk_limit_enabled":"0",
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"Bot - Enhancers : Engagement : Send to Messenger"
   },
   "215":{
      "bulk_limit_enabled":"0",
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"Bot - Enhancers : Engagement : m.me Links"
   },
   "217":{
      "bulk_limit_enabled":"0",
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"Bot - Enhancers : Engagement : Customer Chat Plugin"
   },
   "218":{
      "bulk_limit_enabled":"0",
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"Bot - Enhancers : Sequence Messaging : Message Send"
   },
   "219":{
      "bulk_limit_enabled":"0",
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"Bot - Enhancers : Sequence Messaging Campaign"
   }
}
Project ID: 30
...
In this case, we suggest you to check the error message in report, and if you think it’s not for your message content, but for specific subscribers, you can restart the campaign from where it is left off, by clicking on the option menu & then click Force Resume. ").'"><i class="fas fa-info-circle"></i> </a>';
    ...
    {
        if(!check_module_action_access($module_id=211,2,'check')){
            echo "0";
            exit();
        }
    ...
    public function subscriber_bulk_broadcast_add_action()
    {
      ...
      check_module_action_access($module_id=211,$actions=1);
    ...
    public function subscriber_bulk_broadcast_edit_action()
    {
        ...
        check_module_action_access($module_id=211,$actions=2);
    ...
    {
        check_module_access($module_id=219);
    ...
    {
        $this->ajax_check();
        check_module_action_access($module_id=219,$actions=1);
        $this->is_engagement_exist=$this->engagement_exist();
        $post=$_POST;
    ...
    {
        $this->ajax_check();
        check_module_action_access($module_id=219,$actions=2);
        $this->is_engagement_exist=$this->engagement_exist();
        $post=$_POST;
        foreach ($post as $key => $value) 
        {
    ...
    {
        check_module_action_access($module_id=213,$actions=1);
    ...
    {
        if(!check_module_action_access($module_id=213,$actions=2,'check')){
            echo '0';
            exit();
        }
    ...
    {
        $this->ajax_check();
        check_module_action_access($module_id=214,$actions=1);
    ...
    {
        $this->ajax_check();        
        if(!check_module_action_access($module_id=214,$actions=2,'check')){
            echo '0';
            exit();
        }   
    ...
    {
        $this->ajax_check();
        check_module_action_access($module_id=215,$actions=1);
    ...
    {        
        $this->ajax_check();
        if(!check_module_action_access($module_id=215,$actions=2)){
            echo "0";
            exit();
        }
        $this->is_broadcaster_exist=$this->broadcaster_exist(); 
    ...
    public function customer_chat_add_action()
    {
        ...
        $this->ajax_check();
        check_module_action_access($module_id=217,$actions=1);
    ...
    {
        if(!check_module_access($module_id=217,true)){
            redirect('home/access_forbidden','location');
            exit();
        }
    ...
    public function customer_chat_edit_action()
    {
        ...
        if(check_module_action_access($module_id=217,$actions=2,'check')){
            echo "0";
            exit();
        }
