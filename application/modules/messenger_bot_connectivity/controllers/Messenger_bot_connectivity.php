...
Modules:
{
   "258":{
      ...
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"Bot - Connectivity : JSON API"
   },
   "261":{
      "bulk_limit_enabled":"0",
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"Bot - Connectivity : Webview Builder"
   }
}
Project ID: 31
...
class Messenger_bot_connectivity extends Home
{
	...
	*/
	...
	{
		if (!check_module_action_access($module_id = 261, $actions = [1, 2], 'check')) {
			return $this->customJsonResponse($this->lang->line("You do not have access to perform this action."));
		}
	...
	{
		if (!check_module_action_access($module_id = 261, $actions = 4, 'check')) {
			echo json_encode(['error' => $this->lang->line("You do not have access to perform this action.")]);
			exit;
		}
	...
	{
		check_module_action_access($module_id = 261, $actions = 3);
	...
	public function form_submit()
	{
		...
		$subscriber_info[0]['subscribe_id'] = $subscriber_id;
		...
		if (!empty($subscriber_info)) {
			...
			if ($reply_template_id != "") {
				...
				if (isset($main_message_array['out'])) {
					$temp =  $this->process_message_condition($message_str, $subscriber_info, $subscriber_id);
					$main_message_array = ($temp != "") ? json_decode($temp, true) : array();
				}

				foreach ($main_message_array as $msg) {
					...
					if (isset($subscriber_info[0]['status']) && $subscriber_info[0]['status'] == "1") {
						...
						if (isset($reply_response['error']['message'])) {
							...
							$error_insert_data = array(
								"page_id" => $page_table_id,
								"fb_page_id" => $page_id,
								"user_id" => $user_id,
								"error_message" => $reply_error_message,
								"bot_settings_id" => $bot_settings_id,
	...
	{
		if (!check_module_action_access($module_id = 261, $actions = 3, 'check')) {
			return $this->customJsonResponse($this->lang->line("You do not have access to perform this action."));
			exit;
		}
	...
	*/
	...
	{
		if (!check_module_action_access($module_id = 258, $actions = [1, 2], 'check')) {
			$ext['result'] = 0;
			$ext['msg']    = $this->lang->line("You do not have access to perform this action.");
	...
	public function ajax_view_connector_info()
	{
		...
		$trigger   = implode(',', array_map(function ($el) {
			return $el['trigger_option'];
		}, $trigger_option));
	...
	public function ajax_get_update_connector_info()
	{
		...
		foreach ($page_info as $page_list) {
			...
			if (!empty($this->team_allowed_pages) && !in_array($page_list['id'], $this->team_allowed_pages)) continue;
	...
	{
		if (!check_module_action_access($module_id = 258, $actions = 3, 'check')) {
			echo "0";
			exit();
		}
	...
	*/
	...
	public function activate()
	{
		...
		$sql = array(
			...
			12 => "ALTER TABLE `messenger_bot_thirdparty_webhook` ADD `visual_flow_campaign_id` INT NOT NULL AFTER `page_id`;"
